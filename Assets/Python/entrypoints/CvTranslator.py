...
#メニュー画面項目から外交テキストまで、表示直前のあらゆるテキストがここに送られてくるようです
def finishText(argsList):

	szString = argsList[0]

	#追加、「hoge1@hoge2@hoge3」となっているテキストは@区切りのうちどれか一つをランダム表示
	temp=szString.split(u"@")
	if len(temp)>1:
		#理由は分かりませんがrandomではなく↓が推奨されている模様
		szString = temp[CyGame().getSorenRandNum(len(temp), "")]
