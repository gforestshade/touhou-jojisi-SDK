...
bool CvCity::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost, bool bIgnoreUpgrades) const
{
	...
	//“Œ•û–Ž–Ž—p
	if (GC.getUnitInfo(eUnit).getUnitCombatType() == GC.getInfoTypeForString("UNITCOMBAT_STANDBY")){
        if ( GET_PLAYER(getOwnerINLINE()).getNumTohoUnit() >= GET_PLAYER(getOwnerINLINE()).getNumTohoUnitLimit()   ){
            return false;
        }
        if (isBarbarian()){
            return false;
...
bool CvCity::canConstruct(BuildingTypes eBuilding, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	//“Œ•û–Ž–Ž—p
    if (GC.getGameINLINE().isOption(GAMEOPTION_NO_PYRAMID) && eBuilding == GC.getInfoTypeForString("BUILDING_PYRAMID"))
        return false;
    if (GC.getGameINLINE().isOption(GAMEOPTION_NO_GREAT_WALL) && eBuilding == GC.getInfoTypeForString("BUILDING_GREAT_WALL"))
        return false;
    if (GC.getGameINLINE().isOption(GAMEOPTION_NO_STATUE_OF_ZEUS) && eBuilding == GC.getInfoTypeForString("BUILDING_STATUE_OF_ZEUS"))
        return false;
...
int CvCity::getProductionModifier(ProjectTypes eProject) const
{
	...
	//“Œ•û–Ž–Ž—p
	if (GET_PLAYER(this->getOwner()).hasTrait((TraitTypes)(GC.getInfoTypeForString("TRAIT_UTSUHOLIST")))){
        if (eProject == GC.getInfoTypeForString("PROJECT_MANHATTAN_PROJECT")){
            iMultiplier += 100;
        }
	}
	if (GET_PLAYER(this->getOwner()).hasTrait((TraitTypes)(GC.getInfoTypeForString("TRAIT_REIMULIST")))){
        if (eProject == GC.getInfoTypeForString("PROJECT_THE_INTERNET")){
            iMultiplier += 100;
        }
	}
	if (GET_PLAYER(this->getOwner()).hasTrait((TraitTypes)(GC.getInfoTypeForString("TRAIT_SATORILIST")))){
...
void CvCity::read(FDataStreamBase* pStream)
{
	...
	//“Œ•û–Ž–Ž—p
	pStream->Read(&m_iNumFlowerGardenTurn);
...
void CvCity::write(FDataStreamBase* pStream)
{
	...
	//“Œ•û–Ž–Ž—p
	pStream->Write(m_iNumFlowerGardenTurn);
...
void CvCity::getCityBillboardSizeIconColors(NiColorA& kDotColor, NiColorA& kTextColor) const
{
	...
	/// >>> CGE-LE 1.0.3 (Colorized Slavery Mod 1.0.0)
	NiColorA kGrowing(0.73f, 0.93f, 0.53f, 1);
	/// <<< CGE-LE 1.0.3 (Colorized Slavery Mod 1.0.0)
	...
	{
		/// >>> CGE-LE 1.0.3 (Colorized Slavery Mod 1.0.0)
		bool canHurryPopulation = canHurry((HurryTypes)0, false);
		bool canHurryGold = canHurry((HurryTypes)1, false);

		if (canHurryPopulation && canHurryGold)
		{
			kStagnant = NiColorA(0.4f, 0.5f, 0.4f, 1);
			kShrinking = NiColorA(0.4f, 0.3f, 0.1f, 1);
			kGrowing = NiColorA(0.3f, 0.4f, 0, 1);
			kBlack = kWhite;
		}
		else if (canHurryPopulation)
		{
			kStagnant = NiColorA(0.4f, 0.4f, 0.5f, 1);
			kShrinking = NiColorA(0.4f, 0.1f, 0.2f, 1);
			kGrowing = NiColorA(0, 0.3f, 0.4f, 1);
			kBlack = kWhite;
		}
		else if (canHurryGold)
		{
			kStagnant = NiColorA(0.5f, 0.5f, 0.4f, 1);
			kShrinking = NiColorA(0.4f, 0.2f, 0.1f, 1);
			kGrowing = NiColorA(0.3f, 0.3f, 0, 1);
			kBlack = kWhite;
		}
		/// <<< CGE-LE 1.0.3 (Colorized Slavery Mod 1.0.0)
		if (foodDifference() < 0)
...
NiColorA CvCity::getBarBackgroundColor() const
{
	...
	if ((getTeam() == GC.getGameINLINE().getActiveTeam()))
	{
		if (canHurry((HurryTypes)0, false))
		{
			return NiColorA(0, 0, 0.5f, 0.5f); // blue
		}
	}
	/// <<< CGE-LE 1.0.3 (Colorized Slavery Mod 1.0.0)
...
void CvCity::setFlowerGardenTurn(int iNum){

    m_iNumFlowerGardenTurn = iNum;

}

int CvCity::getFlowerGardenTurn(){

    return m_iNumFlowerGardenTurn;
