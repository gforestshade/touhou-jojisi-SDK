...
public:
	...
	//東方叙事詩用
    DllExport int getNumCombatCombo();
    DllExport void setNumCombatCombo(int iNewNum);
    int getNumLoseCount();
    void setNumLoseCount(int iNum);
    void setNumCastSpellCard(int iNum);
    int getNumCastSpellCard();
    void setNumAcquisSpellPromotion(int iNum);
    int getNumAcquisSpellPromotion();
    int countCardAttackLevel() const;
    void setNumTransformTime(int iNum);
    int getNumTransformTime();
    void setbLoseByPlayer(bool flag);
    bool getbLoseByPlayer();
    void setSinraDelayTurn(int iNum);
    int getSinraDelayTurn();
    void setSpecialNumber(int iNum);
    int getSpecialNumber();
    void setDanmakuKekkai(int iNow,int iMax);
    int getNowDanmakuKekkai();
    int getMaxDanmakuKekkai();
    void setAIPromotionRoute(int iNum);
    int getAIPromotionRoute();
    void countCombatBonus();
    int getTohoExtraCombatPercent();
    int getTohoBaseCombat();
    int getTohoExtraCityDefensePercent();
    void setPower(double iNum);
    double getPower();
    ...
    int countAutoHeal() const;
    int countDodgeBullet() const;
...
protected:
	...
	//東方叙事詩用
    int m_NumCombatCombo; //連続戦闘用
    int m_iNumLoseCount; //倒された回数をカウント
    int m_iNumCastSpellCard; //
    int m_iNumAcquisSpellPromotion;//
    int m_iNumTransformTime; //残り変身時間
    bool m_bLoseByPlayer;// プレイヤーに倒されたかどうかのフラグ
    int m_iSinraDelayTurn;//森羅結界を取得するまでのディレイ
    int m_iSpecialNumber; //何にでもつかってよい特殊な変数　
    int m_iNumNowDanmakuKekkai; //弾幕結界　現在のターン経過数
    int m_iNumMaxDanmakuKekkai; //弾幕結界　持続ターン数
    int m_iAIPromotionRoute; //AIの昇進ルート選択用
    int m_iTohoExtraCombatPercent;//スタックボーナスによる戦闘力補正
    int m_iTohoExtraCityDefensePercent;//びっくりなどによる都市防御補正
    int m_iTohoBaseCombat; //昇進などによる基礎戦闘力の補正
    double m_dPower; //原作のPowerとほぼ同じもの
    double m_dOverflowPower; //Powerのあふれ分
    ...
    int m_iNumSpellPhantasmBreakTime;

    //高速化のためあらかじめ値を用意
    int promCom1;// = (PromotionTypes)GC.getInfoTypeForString("PROMOTION_TOHO_COMBAT1");
    int promCom2;// = (PromotionTypes)GC.getInfoTypeForString("PROMOTION_TOHO_COMBAT2");
    int promCom3;// = (PromotionTypes)GC.getInfoTypeForString("PROMOTION_TOHO_COMBAT3");
    int promCom4;// = (PromotionTypes)GC.getInfoTypeForString("PROMOTION_TOHO_COMBAT4");
    int promCom5;// = (PromotionTypes)GC.getInfoTypeForString("PROMOTION_TOHO_COMBAT5");
