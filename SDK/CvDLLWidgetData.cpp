...
void CvDLLWidgetData::parseHelp(CvWStringBuffer &szBuffer, CvWidgetDataStruct &widgetDataStruct)
{
	...
	//東方叙事詩用
	case WIDGET_PEDIA_JUMP_TO_TOHOUNIT:
		parseUnitHelp(widgetDataStruct, szBuffer);
		break;

    case WIDGET_PEDIA_CAL_UP:
		break;

    case WIDGET_PEDIA_CAL_DOWN:
		break;
...
//		GC.getDiplomacyScreen().handleClick(m_pWidget);
	...
	//東方叙事詩用
    case WIDGET_PEDIA_JUMP_TO_TOHOUNIT:
		doPediaTohoUnitJump(widgetDataStruct);
		break;

    case WIDGET_PEDIA_CAL_UP:
        doPediaCalChange(widgetDataStruct,1);
        break;
    case WIDGET_PEDIA_CAL_DOWN:
        doPediaCalChange(widgetDataStruct,-1);
        break;
...
//東方叙事詩用
void CvDLLWidgetData::doPediaTohoUnitJump(CvWidgetDataStruct &widgetDataStruct)
{
	CyArgsList argsList;

	argsList.add(widgetDataStruct.m_iData1);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "pediaJumpToTohoUnit", argsList.makeFunctionArgs());
}

void CvDLLWidgetData::doPediaCalChange(CvWidgetDataStruct &widgetDataStruct,int iNum)
{
	CyArgsList argsList;

	argsList.add(widgetDataStruct.m_iData1);
	argsList.add(iNum);
	gDLL->getPythonIFace()->callFunction(PYScreensModule, "doPediaCalChange", argsList.makeFunctionArgs());
}
...
void CvDLLWidgetData::parseLeaderHelp(CvWidgetDataStruct &widgetDataStruct, CvWStringBuffer &szBuffer)
{
	if (widgetDataStruct.m_iData2 != -1)
	{
		/// >>> CGE-LE 1.0.3
		// SevoPedia bug fix
		// Pediaは指導者ランダムになることはないので現状はこれで良いはず（本当はpythonの方を修正しないとだけど）
		for (int iCivilization = 0; iCivilization < GC.getNumCivilizationInfos(); iCivilization++)
		{
			if (GC.getCivilizationInfo((CivilizationTypes)iCivilization).isLeaders(widgetDataStruct.m_iData1))
			{
				widgetDataStruct.m_iData2 = iCivilization;
				break;
			}
		}
		/// <<< CGE-LE 1.0.3
