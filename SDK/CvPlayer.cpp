...
bool CvPlayer::canCreate(ProjectTypes eProject, bool bContinue, bool bTestVisible) const
{
	...
	//東方叙事詩用
	if (eProject == GC.getInfoTypeForString("PROJECT_ORENOYOME")){
...
int CvPlayer::getProductionNeeded(ProjectTypes eProject) const
{
	...
	//東方叙事詩用
	if (eProject == GC.getInfoTypeForString("PROJECT_ORENOYOME")){
	    iProductionNeeded = (int)(iProductionNeeded * pow(1.25,this->getNumMadeMyLove_const()));
	}
...
{
	//東方叙事詩用
    int temp = m_iFeatureProductionModifier;
    if (hasTrait((TraitTypes)GC.getInfoTypeForString("TRAIT_RANLIST") ))
        temp += 25;
...
void CvPlayer::read(FDataStreamBase* pStream)
{
	...
	//東方叙事詩用
    pStream->Read(&m_iNumWorldSpell);
    pStream->Read(&m_iNumTohoUnitLimit);
    pStream->Read(&m_iNumTohoUnit);
    pStream->Read(&m_iMysteryiumFlag);
...
void CvPlayer::write(FDataStreamBase* pStream)
{
	...
	//東方叙事詩用
    pStream->Write(m_iNumWorldSpell);
    pStream->Write(m_iNumTohoUnitLimit);
    pStream->Write(m_iNumTohoUnit);
    pStream->Write(m_iMysteryiumFlag);
...
//これで多分植民地独立は無くなるはず
	...
	if (GC.getGameINLINE().isOption(GAMEOPTION_NO_SPLITEMPIRE))
	{
		return false;
	}
...
int CvPlayer::getNumWorldSpell(){

    return m_iNumWorldSpell;

}

void CvPlayer::setNumWorldSpell(int iNum){

    m_iNumWorldSpell = iNum;

}

int CvPlayer::getNumTohoUnitLimit(){

    return m_iNumTohoUnitLimit;

}

void CvPlayer::setNumTohoUnitLimit(int iNum){

    m_iNumTohoUnitLimit = iNum;

}

int CvPlayer::getNumTohoUnit(){

    return m_iNumTohoUnit;

}

void CvPlayer::setNumTohoUnit(int iNum){

    m_iNumTohoUnit = iNum;

}

int CvPlayer::getMysteryiumFlag(){

    return m_iMysteryiumFlag;

}

void CvPlayer::setMysteryiumFlag(int iNum){

    m_iMysteryiumFlag = iNum;

}

int CvPlayer::getAIPromotionRoute(){

    return m_iAIPromotionRoute;

}

void CvPlayer::setAIPromotionRoute(int iNum){

    m_iAIPromotionRoute = iNum;

}
int CvPlayer::getNumMyLove(){

    return m_iNumMyLove;

}

void CvPlayer::setNumMyLove(int iNum){

    m_iNumMyLove = iNum;

}

int CvPlayer::getNumMadeMyLove(){

    return m_iNumMadeMyLove;

}

void CvPlayer::setNumMadeMyLove(int iNum){

    m_iNumMadeMyLove = iNum;

}

int CvPlayer::getNumMadeMyLove_const() const{
