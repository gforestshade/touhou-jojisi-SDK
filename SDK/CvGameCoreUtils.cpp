...
bool isPromotionValid(PromotionTypes ePromotion, UnitTypes eUnit, bool bLeader)
{
	...
	//東方叙事詩用　移動力１でもブリッツがとれる用にコメントアウト
	/*
	if (kUnit.getMoves() == 1)
	{
	...
	*/
	...
	{
		//東方叙事詩用
        //もし前提が内気功１ならば、無条件にTrueを返す
        if (GC.getPromotionInfo(ePromotion).getPrereqPromotion() == GC.getInfoTypeForString("PROMOTION_NAIKIKOU1"))
        {
            return true;
        }
...
/// >>> CGE-LE 1.0.3
void playExclusiveSound(LPCTSTR pszSound, PlayerTypes ePlayer, ExclusiveSoundTypes eExclusiveSound)
{
	...
	static std::map<ExclusiveSoundTypes, time_t> mtLastPlayTime;
	std::map<ExclusiveSoundTypes, time_t>::iterator itLastPlayTime = mtLastPlayTime.find(eExclusiveSound);
	time_t tCurrentTime = timeGetTime();

	if (itLastPlayTime == mtLastPlayTime.end() || tCurrentTime - itLastPlayTime->second > 100)
	{
		gDLL->getInterfaceIFace()->playGeneralSound(pszSound);
		mtLastPlayTime[eExclusiveSound] = tCurrentTime;
	}
}
/// <<< CGE-LE 1.0.3
