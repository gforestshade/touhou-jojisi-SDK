...
void CvGameTextMgr::setUnitHelp(CvWStringBuffer &szString, const CvUnit* pUnit, bool bOneLine, bool bShort)
{
	...
	if (!bOneLine)
	{
		//東方叙事詩用
		//ターン毎の自動経験値獲得確率の表示
		if (pUnit->getUnitCombatType() == GC.getInfoTypeForString("UNITCOMBAT_BOSS") || pUnit->getUnitCombatType() == GC.getInfoTypeForString("UNITCOMBAT_STANDBY")  ){
            szString.append(NEWLINE);
            szString.append(gDLL->getText("TXT_KEY_UNIT_EXP_BY_TURN"));
            wchar tempwchar[16];
            szString.append(_itow(pUnit->countExpByTrun(),tempwchar,10));
            szString.append(L"%");

            if (pUnit->countCardAttackLevel() > 0){
                szString.append(NEWLINE);
                szString.append(gDLL->getText("TXT_KEY_CARD_ATTACK_LEVEL"));
                wchar tempwchar[16];
                szString.append(_itow(pUnit->countCardAttackLevel(),tempwchar,10));
            }
		}

		if (pUnit->countSpellTolerance() > 0){
		    szString.append(NEWLINE);
            szString.append(gDLL->getText("TXT_KEY_SPELL_TOLERANCE"));
            wchar tempwchar[16];
            szString.append(_itow(pUnit->countSpellTolerance(),tempwchar,10));
            szString.append(L"%");
		}

        if (pUnit->countAutoHeal() > 0){
		    szString.append(NEWLINE);
            szString.append(gDLL->getText("TXT_KEY_AUTO_HEAL"));
            wchar tempwchar[16];
            szString.append(_itow(pUnit->countAutoHeal(),tempwchar,10));
            szString.append(L"%");
		}

		//間接攻撃が可能な場合はその攻撃力を表示
		if (pUnit->airRange() > 0 && pUnit->getUnitCombatType() == (UnitCombatTypes)GC.getInfoTypeForString("UNITCOMBAT_BOSS")){
		    szString.append(NEWLINE);
            szString.append(gDLL->getText("TXT_KEY_INDIRECTLY_ATTACK"));
            wchar tempwchar[16];
            szString.append(_itow(pUnit->airMaxCombatStr(NULL)/100,tempwchar,10));
            //szString.append(L"%");
		}

		//弾幕回避率があれば、表示
		if (pUnit->countDodgeBullet() > 0){
		    szString.append(NEWLINE);
            szString.append(gDLL->getText("TXT_KEY_DODGE_BULLET"));
            wchar tempwchar[16];
...
void CvGameTextMgr::setPlotListHelp(CvWStringBuffer &szString, CvPlot* pPlot, bool bOneLine, bool bShort)
{
	...
	int iMaxNumUnits = GC.getDefineINT(bShort ? "MAX_PLOT_LIST_HELP_UNITS_SHORTENED" : "MAX_PLOT_LIST_HELP_UNITS");
	/// <<< CGE-LE 1.0.3
			...
			for (int iI = 0; iI < GC.getNumUnitInfos(); ++iI)
			{
				for (int iJ = 0; iJ < MAX_PLAYERS; iJ++)
				{
					...
					if (aiUnitNumbers[iIndex] > 0)
					{
						/// >>> CGE-LE 1.0.3
						//if (iCount < 5 || bFirst)
						if (iCount < 5 || bFirst || iMaxNumUnits == 0)
						/// <<< CGE-LE 1.0.3
						{
							szString.append(NEWLINE);
							bFirst = false;
